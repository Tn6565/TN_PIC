...
# デプロイ時は st.secrets から取得
REPLICATE_API_TOKEN = st.secrets.get("EXTNREPLICATE") or os.getenv("EXTNREPLICATE")
PIXABAY_API_KEY = st.secrets.get("TNPIXABAY") or os.getenv("TNPIXABAY")

if not REPLICATE_API_TOKEN or not PIXABAY_API_KEY:
    st.error("Replicate または Pixabay の API が設定されていません")
    st.stop()
...
with tabs[1]:
    ...
    prompt = st.text_area("生成プロンプト", "かわいい猫、リアル、自然光、白背景", max_chars=150)
    negative_prompt = st.text_area("除外要素", "blurry, deformed, lowres, bad anatomy")
                ...
                try:
                    ...
                    for i, url in enumerate(urls):
                        ...
                        if 'market_result' in locals():
                            ...
                            if warnings:
                                st.warning(f"画像_{i+1} は既存画像と類似の可能性あり")
                                for w in warnings:
